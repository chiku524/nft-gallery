...
import { MochinStack } from "@/components/mochin-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultMochinSelection,
  findMochinTrait,
  mochinCategoryById,
  mochinTraitCategories,
  mochinTraitSrc,
  randomMochinSelection,
  type MochinSelection,
  type MochinTraitCategory,
} from "@/data/mochin-traits";
import { cn } from "@/lib/utils";

export function MochinsStudio() {
  const [selection, setSelection] = useState<MochinSelection>({ ...defaultMochinSelection });

  const recipe = useMemo(
    () =>
      mochinTraitCategories.map((category) => {
        const trait = findMochinTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: MochinTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <MochinStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack a Mochin</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomMochinSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultMochinSelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {mochinTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={mochinTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {mochinCategoryById(category.id).traits.length}
...
function TraitPick({
  ...
  return (
    ...
    >
      <div className="relative aspect-square bg-[repeating-conic-gradient(#1a1410_0%_25%,#3a2c22_0%_50%)] bg-[length:16px_16px]">
        {src ? (
