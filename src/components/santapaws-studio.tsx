...
import { SantaPawStack } from "@/components/santapaw-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultSantaPawSelection,
  findSantaPawTrait,
  santapawCategoryById,
  santapawTraitCategories,
  santapawTraitSrc,
  randomSantaPawSelection,
  type SantaPawSelection,
  type SantaPawTraitCategory,
} from "@/data/santapaw-traits";
import { cn } from "@/lib/utils";

export function SantaPawsStudio() {
  const [selection, setSelection] = useState<SantaPawSelection>({ ...defaultSantaPawSelection });

  const recipe = useMemo(
    () =>
      santapawTraitCategories.map((category) => {
        const trait = findSantaPawTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: SantaPawTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <SantaPawStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack a giver</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomSantaPawSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultSantaPawSelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {santapawTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={santapawTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {santapawCategoryById(category.id).traits.length}
