...
import { FeltroStack } from "@/components/feltro-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultFeltroSelection,
  findFeltroTrait,
  feltroCategoryById,
  feltroTraitCategories,
  feltroTraitSrc,
  randomFeltroSelection,
  type FeltroSelection,
  type FeltroTraitCategory,
} from "@/data/feltro-traits";
import { cn } from "@/lib/utils";

export function FeltroStudio() {
  const [selection, setSelection] = useState<FeltroSelection>({ ...defaultFeltroSelection });

  const recipe = useMemo(
    () =>
      feltroTraitCategories.map((category) => {
        const trait = findFeltroTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: FeltroTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <FeltroStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Court mixer</p>
            <h2 className="font-heading text-2xl">Stack a suit</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomFeltroSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultFeltroSelection })}>
              Reset
            </Button>
          </div>
        </div>

        <Tabs defaultValue={feltroTraitCategories[0].id}>
          ...
          {feltroTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={feltroTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {feltroCategoryById(category.id).traits.length}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? "border-[#ec5c30] ring-2 ring-[#ec5c30]/40" : "hover:border-foreground/20",
      )}
    >
