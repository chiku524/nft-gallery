...
import { HoodkinStack } from "@/components/hoodkin-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultHoodkinSelection,
  findHoodkinTrait,
  hoodkinCategoryById,
  hoodkinTraitCategories,
  hoodkinTraitSrc,
  randomHoodkinSelection,
  type HoodkinSelection,
  type HoodkinTraitCategory,
} from "@/data/hoodkin-traits";
import { cn } from "@/lib/utils";

export function HoodkinsStudio() {
  const [selection, setSelection] = useState<HoodkinSelection>({ ...defaultHoodkinSelection });

  const recipe = useMemo(
    () =>
      hoodkinTraitCategories.map((category) => {
        const trait = findHoodkinTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: HoodkinTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <HoodkinStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack a Hoodkin</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomHoodkinSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultHoodkinSelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {hoodkinTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={hoodkinTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {hoodkinCategoryById(category.id).traits.length}
