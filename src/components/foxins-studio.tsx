...
import { FoxinStack } from "@/components/foxin-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultFoxinSelection,
  findFoxinTrait,
  foxinCategoryById,
  foxinTraitCategories,
  foxinTraitSrc,
  randomFoxinSelection,
  type FoxinSelection,
  type FoxinTraitCategory,
} from "@/data/foxin-traits";
import { cn } from "@/lib/utils";

export function FoxinsStudio() {
  const [selection, setSelection] = useState<FoxinSelection>({ ...defaultFoxinSelection });

  const recipe = useMemo(
    () =>
      foxinTraitCategories.map((category) => {
        const trait = findFoxinTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: FoxinTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <FoxinStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack a sticker</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomFoxinSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultFoxinSelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {foxinTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {foxinCategoryById(category.id).traits.length}
