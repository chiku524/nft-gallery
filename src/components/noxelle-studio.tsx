...
import { NoxelleStack } from "@/components/noxelle-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultNoxelleSelection,
  findNoxelleTrait,
  noxelleCategoryById,
  noxelleTraitCategories,
  noxelleTraitSrc,
  randomNoxelleSelection,
  type NoxelleSelection,
  type NoxelleTraitCategory,
} from "@/data/noxelle-traits";
import { cn } from "@/lib/utils";

export function NoxelleStudio() {
  const [selection, setSelection] = useState<NoxelleSelection>({ ...defaultNoxelleSelection });

  const recipe = useMemo(
    () =>
      noxelleTraitCategories.map((category) => {
        const trait = findNoxelleTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: NoxelleTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <NoxelleStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Tube mixer</p>
            <h2 className="font-heading text-2xl">Stack a sign</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomNoxelleSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultNoxelleSelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {noxelleTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={noxelleTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {noxelleCategoryById(category.id).traits.length}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? "border-[#e14bff] ring-2 ring-[#e14bff]/40" : "hover:border-foreground/20",
      )}
    >
