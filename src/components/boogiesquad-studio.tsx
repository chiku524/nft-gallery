...
import { BoogieStack } from "@/components/boogie-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultBoogieSelection,
  findBoogieTrait,
  boogieCategoryById,
  boogieTraitCategories,
  boogieTraitSrc,
  randomBoogieSelection,
  type BoogieSelection,
  type BoogieTraitCategory,
} from "@/data/boogie-traits";
import { cn } from "@/lib/utils";

export function BoogieSquadStudio() {
  const [selection, setSelection] = useState<BoogieSelection>({ ...defaultBoogieSelection });

  const recipe = useMemo(
    () =>
      boogieTraitCategories.map((category) => {
        const trait = findBoogieTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: BoogieTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <BoogieStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack a dancer</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomBoogieSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultBoogieSelection })}>
              Reset
            </Button>
          </div>
        </div>

        <Tabs defaultValue={boogieTraitCategories[0].id}>
          ...
          {boogieTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {boogieCategoryById(category.id).traits.length}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? "border-[#ff50b4] ring-2 ring-[#ff50b4]/40" : "hover:border-foreground/20",
      )}
    >
