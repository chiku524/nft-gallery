...
import { PartyPandaStack } from "@/components/party-panda-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultPartyPandaSelection,
  findPartyPandaTrait,
  partyPandaCategoryById,
  partyPandaTraitCategories,
  partyPandaTraitSrc,
  randomPartyPandaSelection,
  type PartyPandaSelection,
  type PartyPandaTraitCategory,
} from "@/data/party-panda-traits";
import { cn } from "@/lib/utils";

export function PartyPandasStudio() {
  const [selection, setSelection] = useState<PartyPandaSelection>({ ...defaultPartyPandaSelection });

  const recipe = useMemo(
    () =>
      partyPandaTraitCategories.map((category) => {
        const trait = findPartyPandaTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: PartyPandaTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <PartyPandaStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack a Party Panda</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomPartyPandaSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultPartyPandaSelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {partyPandaTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={partyPandaTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {partyPandaCategoryById(category.id).traits.length}
