...
import { PurrkinStack } from "@/components/purrkin-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultPurrkinSelection,
  findPurrkinTrait,
  purrkinCategoryById,
  purrkinTraitCategories,
  purrkinTraitSrc,
  randomPurrkinSelection,
  type PurrkinSelection,
  type PurrkinTraitCategory,
} from "@/data/purrkin-traits";
import { cn } from "@/lib/utils";

export function PurrkinsStudio() {
  const [selection, setSelection] = useState<PurrkinSelection>({ ...defaultPurrkinSelection });

  const recipe = useMemo(
    () =>
      purrkinTraitCategories.map((category) => {
        const trait = findPurrkinTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: PurrkinTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <PurrkinStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack a Purrkin</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomPurrkinSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultPurrkinSelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {purrkinTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={purrkinTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {purrkinCategoryById(category.id).traits.length}
...
function TraitPick({
  ...
  return (
    ...
    >
      <div className="relative aspect-square bg-[repeating-conic-gradient(#0d221e_0%_25%,#163e3c_0%_50%)] bg-[length:16px_16px]">
        {src ? (
