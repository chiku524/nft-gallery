...
import { InklingStack } from "@/components/inkling-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultInklingSelection,
  findInklingTrait,
  inklingCategoryById,
  inklingTraitCategories,
  inklingTraitSrc,
  randomInklingSelection,
  type InklingSelection,
  type InklingTraitCategory,
} from "@/data/inkling-traits";
import { cn } from "@/lib/utils";

export function InklingsStudio() {
  const [selection, setSelection] = useState<InklingSelection>({ ...defaultInklingSelection });

  const recipe = useMemo(
    () =>
      inklingTraitCategories.map((category) => {
        const trait = findInklingTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: InklingTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <InklingStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack an Inkling</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomInklingSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultInklingSelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {inklingTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={inklingTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {inklingCategoryById(category.id).traits.length}
