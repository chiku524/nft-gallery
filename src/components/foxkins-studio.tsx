...
import { FoxkinStack } from "@/components/foxkin-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultFoxkinSelection,
  findFoxkinTrait,
  foxkinCategoryById,
  foxkinTraitCategories,
  foxkinTraitSrc,
  randomFoxkinSelection,
  type FoxkinSelection,
  type FoxkinTraitCategory,
} from "@/data/foxkin-traits";
import { cn } from "@/lib/utils";

export function FoxkinsStudio() {
  const [selection, setSelection] = useState<FoxkinSelection>({ ...defaultFoxkinSelection });

  const recipe = useMemo(
    () =>
      foxkinTraitCategories.map((category) => {
        const trait = findFoxkinTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: FoxkinTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <FoxkinStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomFoxkinSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultFoxkinSelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {foxkinTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={foxkinTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {foxkinCategoryById(category.id).traits.length}
