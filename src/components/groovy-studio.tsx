...
import { GroovyStack } from "@/components/groovy-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultGroovySelection,
  findGroovyTrait,
  groovyCategoryById,
  groovyTraitCategories,
  groovyTraitSrc,
  randomGroovySelection,
  type GroovySelection,
  type GroovyTraitCategory,
} from "@/data/groovy-traits";
import { cn } from "@/lib/utils";

export function GroovyStudio() {
  const [selection, setSelection] = useState<GroovySelection>({ ...defaultGroovySelection });

  const recipe = useMemo(
    () =>
      groovyTraitCategories.map((category) => {
        const trait = findGroovyTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: GroovyTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <GroovyStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack a note</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomGroovySelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultGroovySelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {groovyTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={groovyTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {groovyCategoryById(category.id).traits.length}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? "border-[#f4c44a] ring-2 ring-[#f4c44a]/40" : "hover:border-foreground/20",
      )}
    >
      <div className="relative aspect-square bg-[repeating-conic-gradient(#2a0818_0%_25%,#4a1230_0%_50%)] bg-[length:16px_16px]">
        {src ? (
