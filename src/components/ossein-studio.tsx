...
import { OsseinStack } from "@/components/ossein-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultOsseinSelection,
  findOsseinTrait,
  osseinCategoryById,
  osseinTraitCategories,
  osseinTraitSrc,
  randomOsseinSelection,
  type OsseinSelection,
  type OsseinTraitCategory,
} from "@/data/ossein-traits";
import { cn } from "@/lib/utils";

export function OsseinStudio() {
  const [selection, setSelection] = useState<OsseinSelection>({ ...defaultOsseinSelection });

  const recipe = useMemo(
    () =>
      osseinTraitCategories.map((category) => {
        const trait = findOsseinTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: OsseinTraitCategory["id"], traitId: string) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      <div>
        <OsseinStack selection={selection} />
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack a chart</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomOsseinSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultOsseinSelection })}>
              Reset
            </Button>
          </div>
        </div>

        <Tabs defaultValue={osseinTraitCategories[0].id}>
          ...
          {osseinTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 pr-2 sm:grid-cols-3">
                  ...
                  {category.traits.map((trait) => (
                    <TraitPick
                      key={trait.id}
                      name={trait.name}
                      src={osseinTraitSrc(trait.image)}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {osseinCategoryById(category.id).traits.length}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? "border-[#5c769c] ring-2 ring-[#5c769c]/40" : "hover:border-foreground/20",
      )}
    >
