...
import { BirbStack } from "@/components/birb-stack";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  defaultBirbSelection,
  findBirbTrait,
  birbCategoryById,
  birbTraitCategories,
  birbTraitSrc,
  randomBirbSelection,
  type BirbSelection,
  type BirbTraitCategory,
} from "@/data/birb-traits";
import { cn } from "@/lib/utils";

export function BirbsStudio() {
  const [selection, setSelection] = useState<BirbSelection>({ ...defaultBirbSelection });

  const recipe = useMemo(
    () =>
      birbTraitCategories.map((category) => {
        const trait = findBirbTrait(category.id, selection[category.id]);
        return {
  ...
  function setTrait(categoryId: BirbTraitCategory["id"], traitId: string) {
      ...
      <div className="min-w-0 rounded-[1.75rem] border bg-card p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 className="font-heading text-2xl">Stack a citizen</h2>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={() => setSelection(randomBirbSelection())}>
              Random
            </Button>
            <Button type="button" variant="outline" onClick={() => setSelection({ ...defaultBirbSelection })}>
              Reset
            </Button>
          </div>
        </div>
          ...
          {birbTraitCategories.map((category) => (
            <TabsContent key={category.id} value={category.id}>
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                {birbCategoryById(category.id).traits.length}
...
function TraitPick({
  ...
  return (
    ...
    >
      <div className="relative aspect-square bg-[repeating-conic-gradient(#f7f4ef_0%_25%,#e8dcc8_0%_50%)] bg-[length:16px_16px]">
        {src ? (
