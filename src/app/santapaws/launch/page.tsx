...
export const metadata: Metadata = {
  ...
  description: "Upload flattened Santa Paws GIFs to an OpenSea Drop on Base.",
};

const steps = [
  {
    title: "Traits stay layered. The drop is flattened.",
    body: "OpenSea does not stack APNG layers for you. Studio is a live compositor. For a Drop you upload finished GIFs plus a CSV. Run the generator to bake each token onto the shared 12-frame clock. Default generate writes all 7,777 metadata rows and a 16-GIF preview; pass --all to flatten the full supply.",
  },
  {
    title: "Create a Drop on Base",
    body: `In OpenSea Studio, create a Drop on ${santapaws.chain.name} (chain ID ${santapaws.chain.chainId}). Set supply to ${santapaws.supply.toLocaleString()}, paste the project description, and upload the listing kit from public/brand.`,
  },
  {
    title: "Bulk-upload GIFs + CSV",
    body: `Upload every file in generated/santapaws/gifs (1.gif–${santapaws.supply}.gif after --all) and generated/santapaws/SANTAPAWS-opensea-drop.csv. The CSV already uses OpenSea’s required headers (tokenID, name, description, file_name, attributes[Trait]). OpenSea plays GIF, not APNG. Preview the loops, then publish.`,
  },
  {
    title: "Or deploy the ERC-721 yourself",
    body: `contracts/SantaPaws.sol mints token IDs 1–${santapaws.supply.toLocaleString()}. Pin generated/santapaws/json and set the base URI. Import that contract on OpenSea instead of using a Drop if you want a custom mint. Gas is ETH.`,
  },
];

export default function SantaPawsLaunchPage() {
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-4 text-muted-foreground">
        Santa Paws ships as layered APNG traits and as flattened GIF tokens. OpenSea wants
      ...
      <dl className="mt-8 grid gap-3 sm:grid-cols-2">
        {[
          ["Network", santapaws.chain.name],
          ["Chain ID", String(santapaws.chain.chainId)],
          ["Currency", santapaws.chain.currency],
      ...
      <div className="mt-10 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Collection name {santapaws.name}. Symbol {santapaws.symbol}. Category PFPs. The same copy lives in{" "}
          <code className="rounded bg-secondary px-1.5 py-0.5 text-xs">public/metadata/santapaws-description.txt</code>.
        </p>
      ...
      <div className="mt-8 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {[
            ["Logo", "public/brand/logo-santapaws.png", "512×512, 1:1", "/brand/logo-santapaws.png"],
            ["Featured", "public/brand/featured-santapaws.jpg", "1200×800, 3:2", "/brand/featured-santapaws.jpg"],
            ["OpenSea banner", "public/brand/banner-santapaws-opensea.jpg", "2800×700, 4:1", "/brand/banner-santapaws-opensea.jpg"],
            ["Collection GIF", "public/brand/collection-santapaws.gif", "1000×1000 loop", "/brand/collection-santapaws.gif"],
      ...
      <div className="mt-8 rounded-2xl border bg-card p-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          RPC {santapaws.chain.rpcUrl}
          <br />
          Explorer {santapaws.chain.explorer}
        </p>
        <div className="mt-4">
          <AddChainButton chain={santapaws.chain} />
        </div>
      </div>
      ...
      <p className="mt-10 text-sm text-muted-foreground">
        Drop upload notes:{" "}
        <a className="underline" href={santapaws.opensea.metadataGuide} target="_blank" rel="noreferrer">
