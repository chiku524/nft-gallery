...
export const metadata: Metadata = {
  ...
  description: "Upload flattened Mochins GIFs to an OpenSea Drop on Shape.",
};
...
const steps = [
  ...
  {
    title: "Create a Drop on Shape",
    body: `In OpenSea Studio, create a Drop on ${mochins.chain.name} (chain ID ${mochins.chain.chainId}). Set supply to ${mochins.supply.toLocaleString()}, paste the project description, and upload the listing kit from public/brand.`,
  },
  {
    title: "Bulk-upload GIFs + CSV",
    body: `Upload every file in generated/mochins/gifs (1.gif–${mochins.supply}.gif) and generated/mochins/MOCHINS-opensea-drop.csv. The CSV already uses OpenSea’s required headers (tokenID, name, description, file_name, attributes[Trait]). OpenSea plays GIF, not APNG. Preview the loops, then publish.`,
  },
  {
    title: "Or deploy the ERC-721 yourself",
    body: `contracts/Mochins.sol mints token IDs 1–${mochins.supply.toLocaleString()}. Pin generated/mochins/json and set the base URI. Import that contract on OpenSea instead of using a Drop if you want a custom mint. Gas is ETH.`,
  },
];

export default function MochinsLaunchPage() {
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
      <p className="text-xs uppercase tracking-[0.22em] text-muted-foreground">Marketplace</p>
      <h1 className="mt-2 font-heading text-4xl">Launch on OpenSea, Shape.</h1>
      <p className="mt-4 text-muted-foreground">
        Mochins ships as layered APNG traits and as flattened GIF tokens. OpenSea wants the GIF
      ...
      <dl className="mt-8 grid gap-3 sm:grid-cols-2">
        {[
          ["Network", mochins.chain.name],
          ["Chain ID", String(mochins.chain.chainId)],
          ["Currency", mochins.chain.currency],
      ...
      <div className="mt-10 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Collection name {mochins.name}. Symbol {mochins.symbol}. Category PFPs. The same copy lives in{" "}
          <code className="rounded bg-secondary px-1.5 py-0.5 text-xs">public/metadata/mochins-description.txt</code>.
        </p>
      ...
      <div className="mt-8 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {[
            ["Logo", "public/brand/logo-mochins.png", "512×512, 1:1", "/brand/logo-mochins.png"],
            ["Featured", "public/brand/featured-mochins.jpg", "1200×800, 3:2", "/brand/featured-mochins.jpg"],
            ["OpenSea banner", "public/brand/banner-mochins-opensea.jpg", "2800×700, 4:1", "/brand/banner-mochins-opensea.jpg"],
            ["Collection GIF", "public/brand/collection-mochins.gif", "1000×1000 loop", "/brand/collection-mochins.gif"],
      ...
      <div className="mt-8 rounded-2xl border bg-card p-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          RPC {mochins.chain.rpcUrl}
          <br />
          Explorer {mochins.chain.explorer}
        </p>
        <div className="mt-4">
          <AddChainButton chain={mochins.chain} />
        </div>
      </div>
      ...
      <p className="mt-10 text-sm text-muted-foreground">
        Drop upload notes:{" "}
        <a className="underline" href={mochins.opensea.metadataGuide} target="_blank" rel="noreferrer">
          Preparing metadata for your drop
        </a>
        . Live collection:{" "}
        <a className="underline" href={mochins.opensea.collection} target="_blank" rel="noreferrer">
          opensea.io/collection/mochins
        </a>
        . OpenSea on Shape:{" "}
        <a className="underline" href={mochins.opensea.blog} target="_blank" rel="noreferrer">
