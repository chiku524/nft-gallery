...
export const metadata: Metadata = {
  ...
  description: "Upload flattened Groovy Nation GIFs to an OpenSea Drop on Robinhood Chain.",
};
...
const steps = [
  ...
  },
  {
    ...
    body: `In OpenSea Studio, create a Drop on ${groovy.chain.name} (chain ID ${groovy.chain.chainId}). Set supply to ${groovy.supply.toLocaleString()}, paste the project description, and upload the listing kit from public/brand.`,
  },
  {
    title: "Bulk-upload GIFs + CSV",
    body: `Upload every file in generated/groovy/gifs (1.gif–${groovy.supply}.gif) and generated/groovy/GROOVY-opensea-drop.csv. The CSV already uses OpenSea’s required headers (tokenID, name, description, file_name, attributes[Trait]). OpenSea plays GIF, not APNG. Preview the loops, then publish.`,
  },
  {
    title: "Or deploy the ERC-721 yourself",
    body: `contracts/GroovyNation.sol mints token IDs 1–${groovy.supply.toLocaleString()}. Pin generated/groovy/json and set the base URI. Import that contract on OpenSea instead of using a Drop if you want a custom mint. Gas is ETH.`,
  },
];

export default function GroovyLaunchPage() {
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-4 text-muted-foreground">
        Groovy Nation ships as layered APNG traits and as flattened GIF tokens. OpenSea wants
      ...
      <dl className="mt-8 grid gap-3 sm:grid-cols-2">
        {[
          ["Network", groovy.chain.name],
          ["Chain ID", String(groovy.chain.chainId)],
          ["Currency", groovy.chain.currency],
      ...
      <div className="mt-10 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Collection name {groovy.name}. Symbol {groovy.symbol}. Category PFPs. The same copy lives in{" "}
          <code className="rounded bg-secondary px-1.5 py-0.5 text-xs">public/metadata/groovy-description.txt</code>.
        </p>
      ...
      <div className="mt-8 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {[
            ["Logo", "public/brand/logo-groovy.png", "512×512, 1:1", "/brand/logo-groovy.png"],
            ["Featured", "public/brand/featured-groovy.jpg", "1200×800, 3:2", "/brand/featured-groovy.jpg"],
            ["OpenSea banner", "public/brand/banner-groovy-opensea.jpg", "2800×700, 4:1", "/brand/banner-groovy-opensea.jpg"],
            ["Collection GIF", "public/brand/collection-groovy.gif", "1000×1000 loop", "/brand/collection-groovy.gif"],
      ...
      <div className="mt-8 rounded-2xl border bg-card p-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          RPC {groovy.chain.rpcUrl}
          <br />
          Explorer {groovy.chain.explorer}
        </p>
        <div className="mt-4">
          <AddChainButton chain={groovy.chain} />
        </div>
      </div>
      ...
      <ol className="mt-10 space-y-6">
        {steps.map((step, index) => (
          <li key={step.title} className="grid grid-cols-[auto_1fr] gap-4">
            <span className="flex size-8 items-center justify-center rounded-full bg-[#f4c44a] text-sm text-[#2a0818]">
              {index + 1}
            </span>
            <div>
