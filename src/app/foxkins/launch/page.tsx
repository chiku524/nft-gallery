...
export const metadata: Metadata = {
  ...
  description: "Upload flattened Foxkins GIFs to an OpenSea Drop on Base.",
};
...
const steps = [
  ...
  },
  {
    ...
    body: `In OpenSea Studio, create a Drop on ${foxkins.chain.name} (chain ID ${foxkins.chain.chainId}). Set supply to ${foxkins.supply.toLocaleString()}, paste the project description, and upload the listing kit from public/brand.`,
  },
  {
    title: "Bulk-upload GIFs + CSV",
    body: `Upload every file in generated/foxkins/gifs (1.gif–${foxkins.supply}.gif) and generated/foxkins/FOXKINS-opensea-drop.csv. The CSV already uses OpenSea’s required headers (tokenID, name, description, file_name, attributes[Trait]). OpenSea plays GIF, not APNG. Preview the loops, then publish.`,
  },
  {
    title: "Or deploy the ERC-721 yourself",
    body: `contracts/Foxkins.sol mints token IDs 1–${foxkins.supply.toLocaleString()}. Pin generated/foxkins/json and set the base URI. Import that contract on OpenSea instead of using a Drop if you want a custom mint. Gas is ETH.`,
  },
];

export default function FoxkinsLaunchPage() {
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-4 text-muted-foreground">
        Foxkins ships as layered APNG traits and as flattened GIF tokens. OpenSea wants
      ...
      <dl className="mt-8 grid gap-3 sm:grid-cols-2">
        {[
          ["Network", foxkins.chain.name],
          ["Chain ID", String(foxkins.chain.chainId)],
          ["Currency", foxkins.chain.currency],
      ...
      <div className="mt-10 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Collection name {foxkins.name}. Symbol {foxkins.symbol}. Category PFPs. The same copy lives in{" "}
          <code className="rounded bg-secondary px-1.5 py-0.5 text-xs">public/metadata/foxkins-description.txt</code>.
        </p>
      ...
      <div className="mt-8 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {[
            ["Logo", "public/brand/logo-foxkins.png", "512×512, 1:1", "/brand/logo-foxkins.png"],
            ["Featured", "public/brand/featured-foxkins.jpg", "1200×800, 3:2", "/brand/featured-foxkins.jpg"],
            ["OpenSea banner", "public/brand/banner-foxkins-opensea.jpg", "2800×700, 4:1", "/brand/banner-foxkins-opensea.jpg"],
            ["Collection GIF", "public/brand/collection-foxkins.gif", "1000×1000 loop", "/brand/collection-foxkins.gif"],
      ...
      <div className="mt-8 rounded-2xl border bg-card p-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          RPC {foxkins.chain.rpcUrl}
          <br />
          Explorer {foxkins.chain.explorer}
        </p>
        <div className="mt-4">
          <AddChainButton chain={foxkins.chain} />
        </div>
      </div>
      ...
      <p className="mt-10 text-sm text-muted-foreground">
        Drop upload notes:{" "}
        <a className="underline" href={foxkins.opensea.metadataGuide} target="_blank" rel="noreferrer">
          Preparing metadata for your drop
        </a>
        . Live collection:{" "}
        <a className="underline" href={foxkins.opensea.collection} target="_blank" rel="noreferrer">
          opensea.io/collection/foxkins
        </a>
        . OpenSea-compatible chains:{" "}
        <a className="underline" href={foxkins.opensea.blog} target="_blank" rel="noreferrer">
