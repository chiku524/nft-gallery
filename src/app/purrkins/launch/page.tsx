...
export const metadata: Metadata = {
  ...
  description: "Upload flattened Purrkins GIFs to an OpenSea Drop on HyperEVM.",
};
...
const steps = [
  ...
  {
    title: "Create a Drop on HyperEVM",
    body: `In OpenSea Studio, create a Drop on ${purrkins.chain.name} (chain ID ${purrkins.chain.chainId}). Set supply to ${purrkins.supply.toLocaleString()}, paste the project description, and upload the listing kit from public/brand.`,
  },
  {
    title: "Bulk-upload GIFs + CSV",
    body: `Upload every file in generated/purrkins/gifs (1.gif–${purrkins.supply}.gif) and generated/purrkins/PURRKINS-opensea-drop.csv. The CSV already uses OpenSea’s required headers (tokenID, name, description, file_name, attributes[Trait]). OpenSea plays GIF, not APNG. Preview the loops, then publish.`,
  },
  {
    title: "Or deploy the ERC-721 yourself",
    body: `contracts/Purrkins.sol mints token IDs 1–${purrkins.supply.toLocaleString()}. Pin generated/purrkins/json and set the base URI. Import that contract on OpenSea instead of using a Drop if you want a custom mint. Gas is HYPE.`,
  },
];

export default function PurrkinsLaunchPage() {
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
      <p className="text-xs uppercase tracking-[0.22em] text-muted-foreground">Marketplace</p>
      <h1 className="mt-2 font-heading text-4xl">Launch on OpenSea, HyperEVM.</h1>
      <p className="mt-4 text-muted-foreground">
        Purrkins ships as layered APNG traits and as flattened GIF tokens. OpenSea wants the GIF
      ...
      <dl className="mt-8 grid gap-3 sm:grid-cols-2">
        {[
          ["Network", purrkins.chain.name],
          ["Chain ID", String(purrkins.chain.chainId)],
          ["Currency", purrkins.chain.currency],
          ["Supply", purrkins.supply.toLocaleString()],
        ].map(([label, value]) => (
      ...
      <div className="mt-10 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Collection name {purrkins.name}. Symbol {purrkins.symbol}. Category PFPs. The same copy lives in{" "}
          <code className="rounded bg-secondary px-1.5 py-0.5 text-xs">public/metadata/purrkins-description.txt</code>.
        </p>
      ...
      <div className="mt-8 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {[
            ["Logo", "public/brand/logo-purrkins.png", "512×512, 1:1", "/brand/logo-purrkins.png"],
            ["Featured", "public/brand/featured-purrkins.jpg", "1200×800, 3:2", "/brand/featured-purrkins.jpg"],
            ["OpenSea banner", "public/brand/banner-purrkins-opensea.jpg", "2800×700, 4:1", "/brand/banner-purrkins-opensea.jpg"],
            ["Collection GIF", "public/brand/collection-purrkins.gif", "1000×1000 loop", "/brand/collection-purrkins.gif"],
      ...
      <div className="mt-8 rounded-2xl border bg-card p-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          RPC {purrkins.chain.rpcUrl}
          <br />
          Explorer {purrkins.chain.explorer}
        </p>
        <div className="mt-4">
          <AddChainButton chain={purrkins.chain} />
        </div>
      </div>
