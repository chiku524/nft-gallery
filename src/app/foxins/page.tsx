...
import { foxinCombinationCount, foxinTraitCategories } from "@/data/foxin-traits";
import { foxins } from "@/data/foxins";
import { foxinsPath } from "@/lib/foxins";

export default function FoxinsHomePage() {
  return (
    <div>
      <section className="relative h-[420px] overflow-hidden border-b border-border sm:h-[520px]">
        <ApngImage
          src="/brand/banner-foxins.png"
          alt="Five Foxins lined up as round stickers"
        ...
        <div className="absolute inset-x-0 bottom-0 mx-auto w-full max-w-6xl px-4 pb-10 sm:px-6">
          <Badge className="mb-3 bg-background/90 text-foreground">
            {foxins.supply.toLocaleString()} PFP GIFs · {foxins.chain.name}
          </Badge>
          <h1 className="max-w-3xl font-heading text-4xl leading-[1.05] text-white sm:text-6xl">
            {foxins.name}
          </h1>
          <p className="mt-3 max-w-xl text-base text-white/85 sm:text-lg">{foxins.tagline}</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Button asChild size="lg">
              <Link href={foxinsPath("/studio")}>
                Open the layer studio
                <ArrowRight data-icon="inline-end" />
              </Link>
            </Button>
            <Button asChild size="lg" variant="secondary">
              <Link href={foxinsPath("/gallery")}>See sample loops</Link>
            </Button>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6">
        <div className="grid gap-8 lg:grid-cols-12">
          <div className="lg:col-span-5">
            ...
            <dl className="mt-8 grid grid-cols-2 gap-4">
              {[
                ...
                ["Possible combos", foxinCombinationCount().toLocaleString()],
                ["Loop", `${foxins.frames} × ${foxins.frameDurationMs}ms`],
                ["Mint", `${foxins.mintPriceEth} ${foxins.chain.currency}`],
              ].map(([label, value]) => (
          ...
          <div className="grid grid-cols-2 gap-3 lg:col-span-7 sm:grid-cols-4">
            {foxinSamples.slice(0, 8).map((mint) => (
              <Link
                key={mint.id}
                href={foxinsPath("/gallery")}
      ...
      <section className="border-y border-border bg-card/60">
        <div className="mx-auto grid w-full max-w-6xl gap-6 px-4 py-14 sm:px-6 sm:grid-cols-2 lg:grid-cols-3">
          {foxinTraitCategories.map((category) => (
        ...
        <div className="overflow-hidden rounded-[2rem] border bg-[#3a2a22] text-[#fff8ef]">
          <div className="grid gap-8 p-8 md:grid-cols-2 md:p-12">
            ...
            <div className="flex flex-col justify-end gap-3">
              <Button asChild size="lg" variant="secondary">
                <Link href={foxinsPath("/traits")}>Browse every Foxin layer</Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="border-[#fff8ef]/25 bg-transparent text-[#fff8ef]">
                <Link href={foxinsPath("/launch")}>Launch checklist</Link>
              </Button>
            </div>
          </div>
        </div>
