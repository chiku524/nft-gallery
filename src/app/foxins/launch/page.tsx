...
export const metadata: Metadata = {
  ...
  description: "Upload flattened Foxins GIFs to an OpenSea Drop on Base.",
};
...
const steps = [
  ...
  },
  {
    ...
    body: `In OpenSea Studio, create a Drop on ${foxins.chain.name} (chain ID ${foxins.chain.chainId}). Set supply to ${foxins.supply.toLocaleString()}, paste the project description, and upload the listing kit from public/brand.`,
  },
  {
    title: "Bulk-upload GIFs + CSV",
    body: `Upload every file in generated/foxins/gifs (1.gif–${foxins.supply}.gif) and generated/foxins/FOXINS-opensea-drop.csv. The CSV already uses OpenSea’s required headers (tokenID, name, description, file_name, attributes[Trait]). OpenSea plays GIF, not APNG. Preview the loops, then publish.`,
  },
  {
    title: "Or deploy the ERC-721 yourself",
    body: `contracts/Foxins.sol mints token IDs 1–${foxins.supply.toLocaleString()}. Pin generated/foxins/json and set the base URI. Import that contract on OpenSea instead of using a Drop if you want a custom mint. Gas is ETH.`,
  },
];

export default function FoxinsLaunchPage() {
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-4 text-muted-foreground">
        Foxins ships as layered APNG traits and as flattened GIF tokens. OpenSea wants
        the GIF bake of those loops. The APNG stack stays in this repo for the studio and any later
        restack.
      </p>
      <div className="mt-6">
        <Button asChild>
          <a href={foxins.opensea.collection} target="_blank" rel="noreferrer">
      ...
      <dl className="mt-8 grid gap-3 sm:grid-cols-2">
        {[
          ["Network", foxins.chain.name],
          ["Chain ID", String(foxins.chain.chainId)],
          ["Currency", foxins.chain.currency],
          ["Supply", foxins.supply.toLocaleString()],
        ].map(([label, value]) => (
      ...
      <div className="mt-10 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Collection name {foxins.name}. Symbol {foxins.symbol}. Category PFPs. The same copy lives in{" "}
          <code className="rounded bg-secondary px-1.5 py-0.5 text-xs">public/metadata/foxins-description.txt</code>.
        </p>
      ...
      <div className="mt-8 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {[
            ["Logo", "public/brand/logo-foxins.png", "512×512, 1:1", "/brand/logo-foxins.png"],
            ["Featured", "public/brand/featured-foxins.jpg", "1200×800, 3:2", "/brand/featured-foxins.jpg"],
            ["OpenSea banner", "public/brand/banner-foxins-opensea.jpg", "2800×700, 4:1", "/brand/banner-foxins-opensea.jpg"],
            ["Collection GIF", "public/brand/collection-foxins.gif", "1000×1000 loop", "/brand/collection-foxins.gif"],
      ...
      <div className="mt-8 rounded-2xl border bg-card p-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          RPC {foxins.chain.rpcUrl}
          <br />
          Explorer {foxins.chain.explorer}
        </p>
        <div className="mt-4">
          <AddChainButton chain={foxins.chain} />
        </div>
      </div>
      ...
      <p className="mt-10 text-sm text-muted-foreground">
        Drop upload notes:{" "}
        <a className="underline" href={foxins.opensea.metadataGuide} target="_blank" rel="noreferrer">
