...
import { birbCombinationCount, birbTraitCategories } from "@/data/birb-traits";
import { birbs } from "@/data/birbs";
import { birbsPath } from "@/lib/birbs";

export default function BirbsHomePage() {
  return (
    <div>
      <section className="relative h-[420px] overflow-hidden border-b border-border sm:h-[520px]">
        <ApngImage
          src="/brand/banner-birbs.png"
          alt="Five BirbNation robins lined up as round stickers"
          className="absolute inset-0 size-full object-cover object-[center_40%]"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#4a3428] via-[#4a3428]/50 to-transparent" />
        <div className="absolute inset-x-0 bottom-0 mx-auto w-full max-w-6xl px-4 pb-10 sm:px-6">
          <Badge className="mb-3 bg-background/90 text-foreground">
            {birbs.supply.toLocaleString()} PFP GIFs · {birbs.chain.name}
          </Badge>
          <h1 className="max-w-3xl font-heading text-4xl leading-[1.05] text-white sm:text-6xl">
            {birbs.name}
          </h1>
          <p className="mt-3 max-w-xl text-base text-white/85 sm:text-lg">{birbs.tagline}</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Button asChild size="lg">
              <Link href={birbsPath("/studio")}>
                Open the layer studio
                <ArrowRight data-icon="inline-end" />
              </Link>
            </Button>
            <Button asChild size="lg" variant="secondary">
              <Link href={birbsPath("/gallery")}>See sample loops</Link>
            </Button>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6">
        <div className="grid gap-8 lg:grid-cols-12">
          <div className="lg:col-span-5">
            ...
            <h2 className="mt-2 font-heading text-3xl sm:text-4xl">One sphere. A nation of vibes.</h2>
            ...
            <dl className="mt-8 grid grid-cols-2 gap-4">
              {[
                ...
                ["Possible combos", birbCombinationCount().toLocaleString()],
                ["Loop", `${birbs.frames} × ${birbs.frameDurationMs}ms`],
                ["Mint", `${birbs.mintPriceEth} ${birbs.chain.currency}`],
              ].map(([label, value]) => (
          ...
          <div className="grid grid-cols-2 gap-3 lg:col-span-7 sm:grid-cols-4">
            {birbSamples.slice(0, 8).map((mint) => (
              <Link
                key={mint.id}
                href={birbsPath("/gallery")}
      ...
      <section className="border-y border-border bg-card/60">
        <div className="mx-auto grid w-full max-w-6xl gap-6 px-4 py-14 sm:px-6 sm:grid-cols-2 lg:grid-cols-4">
          {birbTraitCategories.map((category) => (
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6">
        <div className="overflow-hidden rounded-[2rem] border bg-[#4a3428] text-[#fff8ef]">
          <div className="grid gap-8 p-8 md:grid-cols-2 md:p-12">
            <div>
              <p className="text-xs uppercase tracking-[0.22em] text-[#e07a3a]">How the stack works</p>
              <h2 className="mt-2 font-heading text-4xl">Studio plays layers. The drop flattens them.</h2>
              <p className="mt-4 text-[#fff8ef]/75">
                Each trait is its own looping APNG. The studio stacks those files so fields, plumage,
                mugs, and accents keep blinking. Minted tokens composite the same 12 frames into one GIF
                OpenSea can list on Robinhood Chain.
              </p>
            </div>
            <div className="flex flex-col justify-end gap-3">
              <Button asChild size="lg" variant="secondary">
                <Link href={birbsPath("/traits")}>Browse every BirbNation layer</Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="border-[#fff8ef]/25 bg-transparent text-[#fff8ef]">
                <Link href={birbsPath("/launch")}>Launch checklist</Link>
              </Button>
            </div>
          </div>
        </div>
