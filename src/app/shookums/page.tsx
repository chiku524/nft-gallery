...
import { shookumCombinationCount, shookumTraitCategories } from "@/data/shookum-traits";
import { shookums } from "@/data/shookums";
import { shookumsPath } from "@/lib/shookums";

export default function ShookumsHomePage() {
  return (
    <div>
      <section className="relative h-[420px] overflow-hidden border-b border-border sm:h-[520px]">
        <ApngImage
          src="/brand/banner-shookums.png"
          alt="Five Halloween Shook'ums lined up as round stickers"
          className="absolute inset-0 size-full object-cover object-[center_40%]"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#1c1830] via-[#1c1830]/50 to-transparent" />
        <div className="absolute inset-x-0 bottom-0 mx-auto w-full max-w-6xl px-4 pb-10 sm:px-6">
          <Badge className="mb-3 bg-background/90 text-foreground">
            {shookums.supply.toLocaleString()} PFP GIFs · {shookums.chain.name}
          </Badge>
          <h1 className="max-w-3xl font-heading text-4xl leading-[1.05] text-white sm:text-6xl">
            {shookums.name}
          </h1>
          <p className="mt-3 max-w-xl text-base text-white/85 sm:text-lg">{shookums.tagline}</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Button asChild size="lg">
              <Link href={shookumsPath("/studio")}>
                Open the layer studio
                <ArrowRight data-icon="inline-end" />
              </Link>
            </Button>
            <Button asChild size="lg" variant="secondary">
              <Link href={shookumsPath("/gallery")}>See sample loops</Link>
            </Button>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6">
        <div className="grid gap-8 lg:grid-cols-12">
          <div className="lg:col-span-5">
            ...
            <h2 className="mt-2 font-heading text-3xl sm:text-4xl">Three sheets. One haunt.</h2>
            <div className="mt-4 space-y-4 text-muted-foreground">
              {shookums.story.split("\n\n").map((paragraph) => (
            ...
            <dl className="mt-8 grid grid-cols-2 gap-4">
              {[
                ...
                ["Possible combos", shookumCombinationCount().toLocaleString()],
                ["Loop", `${shookums.frames} × ${shookums.frameDurationMs}ms`],
                ["Mint", `${shookums.mintPriceEth} ${shookums.chain.currency}`],
              ].map(([label, value]) => (
          ...
          <div className="grid grid-cols-2 gap-3 lg:col-span-7 sm:grid-cols-4">
            {shookumSamples.slice(0, 8).map((mint) => (
              <Link
                key={mint.id}
                href={shookumsPath("/gallery")}
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6">
        <div className="overflow-hidden rounded-[2rem] border bg-[#1c1830] text-[#fff8ef]">
          <div className="grid gap-8 p-8 md:grid-cols-2 md:p-12">
            <div>
              <p className="text-xs uppercase tracking-[0.22em] text-[#e07028]">How the stack works</p>
              <h2 className="mt-2 font-heading text-4xl">Studio plays layers. The drop flattens them.</h2>
              <p className="mt-4 text-[#fff8ef]/75">
                Each trait is its own looping APNG. The studio stacks those files so nights, sheets,
                mugs, hats, wraps, and charms keep blinking. The sheet stays locked. Minted tokens
                composite the same 12 frames into one GIF OpenSea can list on Abstract.
              </p>
            </div>
            <div className="flex flex-col justify-end gap-3">
              <Button asChild size="lg" variant="secondary">
                <Link href={shookumsPath("/traits")}>Browse every Shook&apos;um layer</Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="border-[#fff8ef]/25 bg-transparent text-[#fff8ef]">
                <Link href={shookumsPath("/launch")}>Launch checklist</Link>
              </Button>
            </div>
          </div>
        </div>
