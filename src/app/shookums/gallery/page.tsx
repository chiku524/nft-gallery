...
export const metadata: Metadata = {
  title: "Gallery",
  description: "Signature Halloween Shook'ums looks stacked from looping PFP layers.",
};

const extraPreviews = [9, 10, 11, 12, 13, 14, 15, 16];

export default function ShookumsGalleryPage() {
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-3 max-w-2xl text-muted-foreground">
        ...
        on the same {shookums.frames}-frame clock. The rest of the {shookums.supply.toLocaleString()} lives
        in <code className="rounded bg-secondary px-1.5 py-0.5 text-sm">generated/shookums/gifs</code> after
        you run the generator.
      </p>

      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {shookumSamples.map((mint) => (
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {extraPreviews.map((id) => (
          <article key={id} className="overflow-hidden rounded-2xl border bg-card">
            <ApngImage
              src={`/shookums-preview/${id}.gif?v=1`}
              alt={`Shook'um #${id}`}
              width={512}
              height={512}
              className="aspect-square w-full object-cover"
