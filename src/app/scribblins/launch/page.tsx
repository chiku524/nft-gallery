...
export const metadata: Metadata = {
  ...
  description: "Upload flattened Scribblins GIFs to an OpenSea Drop on Base.",
};
...
const steps = [
  ...
  },
  {
    ...
    body: `In OpenSea Studio, create a Drop on ${scribblins.chain.name} (chain ID ${scribblins.chain.chainId}). Set supply to ${scribblins.supply.toLocaleString()}, paste the project description, and upload the listing kit from public/brand.`,
  },
  {
    title: "Bulk-upload GIFs + CSV",
    body: `Upload every file in generated/scribblins/gifs (1.gif–${scribblins.supply}.gif) and generated/scribblins/SCRIBBLINS-opensea-drop.csv. The CSV already uses OpenSea’s required headers (tokenID, name, description, file_name, attributes[Trait]). OpenSea plays GIF, not APNG. Preview the loops, then publish.`,
  },
  {
    title: "Or deploy the ERC-721 yourself",
    body: `contracts/Scribblins.sol mints token IDs 1–${scribblins.supply.toLocaleString()}. Pin generated/scribblins/json and set the base URI. Import that contract on OpenSea instead of using a Drop if you want a custom mint. Gas is ETH.`,
  },
];

export default function ScribblinsLaunchPage() {
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-4 text-muted-foreground">
        Scribblins ships as layered APNG traits and as flattened GIF tokens. OpenSea wants
        the GIF bake of those loops. The APNG stack stays in this repo for the studio and any later
        restack.
      </p>
      <div className="mt-6">
        <Button asChild>
          <a href={scribblins.opensea.collection} target="_blank" rel="noreferrer">
      ...
      <dl className="mt-8 grid gap-3 sm:grid-cols-2">
        {[
          ["Network", scribblins.chain.name],
          ["Chain ID", String(scribblins.chain.chainId)],
          ["Currency", scribblins.chain.currency],
          ["Supply", scribblins.supply.toLocaleString()],
        ].map(([label, value]) => (
      ...
      <div className="mt-10 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Collection name {scribblins.name}. Symbol {scribblins.symbol}. Category PFPs. The same copy lives in{" "}
          <code className="rounded bg-secondary px-1.5 py-0.5 text-xs">public/metadata/scribblins-description.txt</code>.
        </p>
      ...
      <div className="mt-8 rounded-[1.75rem] border bg-card p-6 sm:p-8">
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {[
            ["Logo", "public/brand/logo-scribblins.png", "512×512, 1:1", "/brand/logo-scribblins.png"],
            ["Featured", "public/brand/featured-scribblins.jpg", "1200×800, 3:2", "/brand/featured-scribblins.jpg"],
            ["OpenSea banner", "public/brand/banner-scribblins-opensea.jpg", "2800×700, 4:1", "/brand/banner-scribblins-opensea.jpg"],
            ["Collection GIF", "public/brand/collection-scribblins.gif", "1000×1000 loop", "/brand/collection-scribblins.gif"],
      ...
      <div className="mt-8 rounded-2xl border bg-card p-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          RPC {scribblins.chain.rpcUrl}
          <br />
          Explorer {scribblins.chain.explorer}
        </p>
        <div className="mt-4">
          <AddChainButton chain={scribblins.chain} />
        </div>
      </div>
      ...
      <p className="mt-10 text-sm text-muted-foreground">
        Drop upload notes:{" "}
        <a className="underline" href={scribblins.opensea.metadataGuide} target="_blank" rel="noreferrer">
