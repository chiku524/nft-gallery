...
export const metadata: Metadata = {
  title: "Gallery",
  description: "Signature Loopkins stacked from APNG trait layers.",
};
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-2 max-w-2xl text-muted-foreground">
        Eight more baked loops. Open them as files and they keep playing — these are APNGs, not stills.
      </p>
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {extraPreviews.map((id) => (
          <article key={id} className="overflow-hidden rounded-2xl border bg-card">
            <ApngImage
              src={`/generated-preview/${id}.png`}
              alt={`Loopkin #${id}`}
              width={512}
              height={512}
              className="aspect-square w-full object-cover"
