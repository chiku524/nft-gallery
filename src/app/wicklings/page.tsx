...
import { wicklingCombinationCount, wicklingTraitCategories } from "@/data/wickling-traits";
import { wicklings } from "@/data/wicklings";
import { wicklingsPath } from "@/lib/wicklings";

export default function WicklingsHomePage() {
  return (
    <div>
      <section className="relative h-[420px] overflow-hidden border-b border-border sm:h-[520px]">
        <ApngImage
          src="/brand/banner-wicklings.png"
          alt="Five Wicklings hanging against alley, festival, and moonlight"
          className="absolute inset-0 size-full object-cover object-[center_40%]"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#0b1024] via-[#0b1024]/55 to-transparent" />
        <div className="absolute inset-x-0 bottom-0 mx-auto w-full max-w-6xl px-4 pb-10 sm:px-6">
          <Badge className="mb-3 bg-background/90 text-foreground">
            {wicklings.supply.toLocaleString()} PFP GIFs · {wicklings.chain.name}
          </Badge>
          <h1 className="max-w-3xl font-heading text-4xl leading-[1.05] text-white sm:text-6xl">
            {wicklings.name}
          </h1>
          <p className="mt-3 max-w-xl text-base text-white/85 sm:text-lg">{wicklings.tagline}</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Button asChild size="lg">
              <Link href={wicklingsPath("/studio")}>
                Open the layer studio
                <ArrowRight data-icon="inline-end" />
              </Link>
            </Button>
            <Button asChild size="lg" variant="secondary">
              <Link href={wicklingsPath("/gallery")}>See sample loops</Link>
            </Button>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6">
        <div className="grid gap-8 lg:grid-cols-12">
          <div className="lg:col-span-5">
            ...
            <h2 className="mt-2 font-heading text-3xl sm:text-4xl">Paper lanterns, looping like Loopkins.</h2>
            <div className="mt-4 space-y-4 text-muted-foreground">
              {wicklings.story.split("\n\n").map((paragraph) => (
            ...
            <dl className="mt-8 grid grid-cols-2 gap-4">
              {[
                ...
                ["Possible combos", wicklingCombinationCount().toLocaleString()],
                ["Loop", `${wicklings.frames} × ${wicklings.frameDurationMs}ms`],
              ].map(([label, value]) => (
          ...
          <div className="grid grid-cols-2 gap-3 lg:col-span-7 sm:grid-cols-4">
            {wicklingSamples.slice(0, 8).map((mint) => (
              <Link
                key={mint.id}
                href={wicklingsPath("/gallery")}
      ...
      <section className="border-y border-border bg-card/60">
        <div className="mx-auto grid w-full max-w-6xl gap-6 px-4 py-14 sm:px-6 md:grid-cols-3">
          {wicklingTraitCategories.map((category) => (
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6">
        <div className="overflow-hidden rounded-[2rem] border bg-[#0b1024] text-[#f4f1ff]">
          <div className="grid gap-8 p-8 md:grid-cols-2 md:p-12">
            <div>
              <p className="text-xs uppercase tracking-[0.22em] text-[#ffd36a]">How the stack works</p>
              <h2 className="mt-2 font-heading text-4xl">Studio plays layers. The drop flattens them.</h2>
              <p className="mt-4 text-[#f4f1ff]/75">
                Each trait is its own looping APNG. The studio stacks those files so nights, flames, and
                moths keep moving. Minted tokens composite the same 12 frames into one GIF OpenSea can
                list on Arbitrum.
              </p>
            </div>
            <div className="flex flex-col justify-end gap-3">
              <Button asChild size="lg" variant="secondary">
                <Link href={wicklingsPath("/traits")}>Browse every lantern layer</Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="border-[#f4f1ff]/25 bg-transparent text-[#f4f1ff]">
                <Link href={wicklingsPath("/launch")}>Launch checklist</Link>
              </Button>
            </div>
          </div>
        </div>
