...
import { afterimageWorks, afterimages, getAfterimageWork } from "@/data/afterimages";
import { afterimagesPath } from "@/lib/afterimages";

export function generateStaticParams() {
  return afterimageWorks.map((work) => ({ id: String(work.id) }));
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  const { id } = await params;
  const work = getAfterimageWork(Number(id));
  if (!work) {
...
export default async function AfterimageWorkPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const work = getAfterimageWork(Number(id));
  if (!work) {
    notFound();
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
      <Link href={afterimagesPath("/gallery")} className="text-sm text-muted-foreground hover:text-foreground">
        ← Back to the wall
      </Link>
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-12">
        ...
        <div className="lg:col-span-5">
          <Badge variant="secondary">
            Token #{work.id} · {afterimages.edition}
          </Badge>
          ...
          <p className="mt-6 text-sm text-muted-foreground">
            ...
            {afterimages.canvas}×{afterimages.canvas} APNG · {afterimages.frames} frames
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            {prev ? (
              <Button asChild variant="secondary">
                <Link href={afterimagesPath(`/${prev.id}`)}>
            ...
            {next ? (
              <Button asChild>
                <Link href={afterimagesPath(`/${next.id}`)}>
                  {next.title}
