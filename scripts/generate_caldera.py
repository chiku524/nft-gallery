#!/usr/bin/env python3
"""Compose Caldera tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 10,000 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/caldera-traits/.
"""
...
from build_caldera import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Match #{token_id}",
        "description": "A looping hotel matchbook from Caldera. Offset-printed cardboard on a cafe table. The flame is the loop. Minted for $0.25 on Robinhood Chain.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Caldera layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "CALDERA-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        "# Caldera OpenSea kit\n\n"
        f"{count:,} flattened hotel-matchbook loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        f"- Supply: `{TOTAL}`\n"
        f"- Mint: `${MINT_PRICE_USD}` (about `{MINT_PRICE_ETH}` ETH at listing; set the live ETH amount in OpenSea)\n"
...
def main() -> None:
    ...
    print(f"Baking {count} Caldera tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Caldera GIFs to generated/caldera/gifs ({gib:.2f} GiB)")
