#!/usr/bin/env python3
"""Compose Inklings tokens from layered ink-wash APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 5,555 on the shared 16-frame clock.

Drop files are 512×512 APNGs so a 5,555 pack stays manageable.
Studio traits stay 640×640 in public/inklings-traits/.
"""
...
from build_inklings import (  # noqa: E402
    DURATION_MS,
    FRAMES,
    PREVIEW_DIR,
    SIGNATURES,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        ...
        "animation_loop": True,
        "compiler": "Inklings ink-wash stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "INKLINGS-opensea-drop.csv"):
...
def main() -> None:
    ...
    print(f"Baking {count} Inklings at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Inklings to generated/inklings ({total_bytes / 1_000_000:.1f} MB)")
