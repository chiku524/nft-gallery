#!/usr/bin/env python3
"""Compose Noxelle tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 10,000 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/noxelle-traits/.
"""
...
from build_noxelle import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Tube #{token_id}",
        "description": "A looping neon-tube dancer from Noxelle. Bent glass. Noble gas. One clock. Minted for $0.25 on Robinhood Chain.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Noxelle layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "NOXELLE-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        "# Noxelle OpenSea kit\n\n"
        f"{count:,} flattened neon-tube loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "3. Upload `NOXELLE-opensea-drop.csv` (or `opensea-metadata.csv`). "
        ...
        "`public/noxelle-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Baking {count} Noxelle tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Noxelle GIFs to generated/noxelle/gifs ({gib:.2f} GiB)")
