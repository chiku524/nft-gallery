...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "PURRKINS-opensea-drop.csv"):
...
def main() -> None:
    ...
    print(f"Baking {count} Purrkins at {DROP_SIZE}px with {args.workers} workers…")
