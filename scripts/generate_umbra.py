#!/usr/bin/env python3
"""Compose Umbra tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 8,888 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/umbra-traits/.
"""
...
from build_umbra import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def load_cache() -> dict[tuple[str, str], list[Image.Image]]:
    ...
    for category, traits in TRAIT_SPEC.items():
        for trait_id, _name, rarity in traits:
            if trait_id == "none":
                continue
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Shade #{token_id}",
        "description": "A looping shadow puppet from Umbra. Leather silhouette. Hinged limbs. One lamp. Free mint on Robinhood Chain.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Umbra layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "UMBRA-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        "# Umbra OpenSea kit\n\n"
        f"{count:,} flattened shadow-puppet loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "3. Upload `UMBRA-opensea-drop.csv` (or `opensea-metadata.csv`). "
        ...
        "`public/umbra-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Baking {count} Umbra tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Umbra GIFs to generated/umbra/gifs ({gib:.2f} GiB)")
