#!/usr/bin/env python3
"""Compose Birbs tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 2,222 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/birbs-traits/.
"""
...
from build_birbs import (  # noqa: E402
    DURATION_MS,
    FRAMES,
    PREVIEW_DIR,
    SIGNATURES,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        ...
        "animation_loop": True,
        "compiler": "BirbNation layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "BIRBS-opensea-drop.csv"):
...
def main() -> None:
    ...
    print(f"Baking {count} BirbNation tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} BirbNation GIFs to generated/birbs/gifs ({gib:.2f} GiB)")
