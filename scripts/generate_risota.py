#!/usr/bin/env python3
"""Compose Risota tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 8,888 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/risota-traits/.
"""
...
from build_risota import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Print #{token_id}",
        "description": "A looping risograph dancer from Risota. Spot ink. Misregister. One clock. Minted on Robinhood Chain.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Risota layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "RISOTA-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        "# Risota OpenSea kit\n\n"
        f"{count:,} flattened risograph loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "3. Upload `RISOTA-opensea-drop.csv` (or `opensea-metadata.csv`). "
        ...
        "`public/risota-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Baking {count} Risota tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Risota GIFs to generated/risota/gifs ({gib:.2f} GiB)")
