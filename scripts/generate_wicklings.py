...
from build_wicklings import (  # noqa: E402
    DURATION_MS,
    FRAMES,
    PREVIEW_DIR,
    SIGNATURES,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Wickling #{token_id}",
        "description": "A looping paper-lantern PFP. The wick blinks. The paper never cools. Minted on Arbitrum.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Wicklings layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "WICKLINGS-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        ...
        "Studio trait layers stay in `public/wicklings-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Baking {count} Wicklings at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Wicklings to generated/wicklings ({total_bytes / 1_000_000:.1f} MB)")
