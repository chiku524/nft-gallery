#!/usr/bin/env python3
"""Compose Hoodkins tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 10,000 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/hoodkins-traits/.
"""
...
from build_hoodkins import (  # noqa: E402
    DURATION_MS,
    FRAMES,
    PREVIEW_DIR,
    SIGNATURES,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Hoodkin #{token_id}",
        "description": "A looping chibi-raccoon PFP. Bandit mask. Ears twitch. Eyes blink. Minted on Robinhood Chain.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Hoodkins layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "HOODKINS-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        ...
        f"{count:,} flattened chibi-raccoon loops at {DROP_SIZE}×{DROP_SIZE}, {FRAMES} frames, {DURATION_MS}ms.\n\n"
...
def main() -> None:
    ...
    print(f"Baking {count} Hoodkins at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Hoodkins GIFs to generated/hoodkins/gifs ({gib:.2f} GiB)")
