#!/usr/bin/env python3
"""Compose Boogie Squad tokens from layered APNG traits.

Always writes deterministic metadata for the full 10,000.
Default: bake 16 signature-preview GIFs.
Pass --all to flatten every token onto the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/boogiesquad-traits/.
"""
...
from build_boogiesquad import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    NAME,
...
from paint_kit import save_apng  # noqa: E402
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        ...
        "description": f"A looping jelly-cel dancer from {NAME}. Mixed types. One groove. Minted on Robinhood Chain.",
...
def bake_one(job: tuple[int, dict[str, str]]) -> tuple[int, int]:
    ...
    apng_dest = IMAGE_DIR / f"{token_id}.png"
    need_gif = not gif_dest.exists() or gif_dest.stat().st_size == 0
    need_apng = not apng_dest.exists() or apng_dest.stat().st_size == 0
    if need_gif or need_apng:
        frames = compose_cached(selection)
        if need_gif:
            save_loop_gif(frames, gif_dest, DURATION_MS, colors=GIF_COLORS)
        if need_apng:
            save_apng(frames, apng_dest)
...
def write_sidecar(baked: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
        ...
        json.dumps(
            {
                ...
                "note": (
                    None
                    if baked >= TOTAL
                    else f"Preview GIF bake of {baked}. Run `python3 scripts/generate_boogiesquad.py --all` for all {TOTAL:,} GIFs."
                ),
            },
            indent=2,
        )
    ...
    (OUT / "README.md").write_text(
        ...
        f"{baked:,} flattened jelly-cel dance loops baked at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "python3 scripts/generate_boogiesquad.py        # full metadata + 16 preview GIFs\n"
        "python3 scripts/generate_boogiesquad.py --all  # full 10,000 GIFs\n"
        ...
        f"`public/{SLUG}-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Writing metadata for all {TOTAL} {NAME}…")
    ...
    print(f"Baking {bake_count} {NAME} GIFs at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {TOTAL} {NAME} metadata files and {bake_count} GIFs to generated/{SLUG}/ ({gib:.2f} GiB baked)")
    if bake_count < TOTAL:
        print(f"Preview pack only. Bake the rest with: python3 scripts/generate_boogiesquad.py --all")
