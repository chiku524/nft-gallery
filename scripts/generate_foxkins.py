#!/usr/bin/env python3
"""Compose Foxkins tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 5,555 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/foxkins-traits/.
"""
...
from build_foxkins import (  # noqa: E402
    DURATION_MS,
    FRAMES,
    PREVIEW_DIR,
    SIGNATURES,
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "FOXKINS-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        ...
        f"{count:,} flattened Foxkins loops at {DROP_SIZE}×{DROP_SIZE}, {FRAMES} frames, {DURATION_MS}ms.\n\n"
        "Listing kit (OpenSea collection page):\n"
        "- Name: Foxkins · Symbol: FOXK · Category: PFPs · Chain: Base (8453)\n"
        "- Description: `public/metadata/foxkins-description.txt`\n"
        "- Logo: `public/brand/logo-foxkins.png` (512×512)\n"
        "- Featured: `public/brand/featured-foxkins.jpg` (1200×800)\n"
        "- Banner: `public/brand/banner-foxkins-opensea.jpg` (2800×700)\n"
        "- Collection GIF: `public/brand/collection-foxkins.gif` (1000×1000)\n\n"
        f"Drop upload: every file in `gifs/` (1.gif–{count}.gif) plus `FOXKINS-opensea-drop.csv` "
        ...
        "Studio trait layers stay in `public/foxkins-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    if args.hats_only:
        ...
        print(f"Baking {count} Foxkins tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Foxkins GIFs to generated/foxkins/gifs ({gib:.2f} GiB)")
