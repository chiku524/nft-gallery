#!/usr/bin/env python3
"""Compose Kamiori tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 8,888 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/kamiori-traits/.
"""
...
from build_kamiori import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Fold #{token_id}",
        "description": "A looping origami sheet from Kamiori. Polygonal facets. A corner that lifts. One clock. Free mint on Robinhood Chain.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Kamiori layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "KAMIORI-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        "# Kamiori OpenSea kit\n\n"
        f"{count:,} flattened origami loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "3. Upload `KAMIORI-opensea-drop.csv` (or `opensea-metadata.csv`). "
        ...
        "`public/kamiori-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Baking {count} Kamiori tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Kamiori GIFs to generated/kamiori/gifs ({gib:.2f} GiB)")
