#!/usr/bin/env python3
"""Compose Cera tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 10,000 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/cera-traits/.
"""
...
from build_cera import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Lamp #{token_id}",
        "description": "A looping lava lamp from Cera. Paraffin in a glass flask on a nightstand. The wax is the loop. Minted for $0.25 on Robinhood Chain.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Cera layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "CERA-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        "# Cera OpenSea kit\n\n"
        f"{count:,} flattened lava-lamp loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "3. Upload `CERA-opensea-drop.csv` (or `opensea-metadata.csv`). "
        ...
        "`public/cera-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Baking {count} Cera tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Cera GIFs to generated/cera/gifs ({gib:.2f} GiB)")
