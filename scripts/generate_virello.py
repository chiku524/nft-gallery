#!/usr/bin/env python3
"""Compose Virello tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 10,000 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/virello-traits/.
"""
...
from build_virello import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Toy #{token_id}",
        "description": "A looping wind-up tin toy from Virello. Lithographed enamel on stamped metal. The winding key is the loop. Minted for $0.25 on Robinhood Chain.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Virello layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "VIRELLO-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        "# Virello OpenSea kit\n\n"
        f"{count:,} flattened wind-up tin-toy loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "3. Upload `VIRELLO-opensea-drop.csv` (or `opensea-metadata.csv`). "
        ...
        "`public/virello-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Baking {count} Virello tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Virello GIFs to generated/virello/gifs ({gib:.2f} GiB)")
