...
from build_opaline import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "OPALINE-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        "# Opaline OpenSea kit\n\n"
        f"{count:,} flattened smoked-glass loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "3. Upload `OPALINE-opensea-drop.csv` (or `opensea-metadata.csv`). "
        ...
        "`public/opaline-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Baking {count} Opaline tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Opaline GIFs to generated/opaline/gifs ({gib:.2f} GiB)")
