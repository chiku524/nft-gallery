#!/usr/bin/env python3
"""Compose Groovy Nation tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 8,888 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/groovy-traits/.
"""
...
from build_groovy import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        ...
        "animation_loop": True,
        "compiler": "Groovy Nation layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "GROOVY-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        ...
        f"{count:,} flattened musical-note loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "3. Upload `GROOVY-opensea-drop.csv` (or `opensea-metadata.csv`). "
        ...
        "`public/groovy-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Baking {count} Groovy Nation tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Groovy Nation GIFs to generated/groovy/gifs ({gib:.2f} GiB)")
