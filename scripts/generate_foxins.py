#!/usr/bin/env python3
"""Compose Foxins tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 5,555 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/foxins-traits/.
"""
...
from build_foxins import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    PREVIEW_DIR,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Foxin #{token_id}",
        "description": "A looping bold-graphic fox from Foxins. Three pelts. One sticker. One clock. Minted on Base.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Foxins layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "FOXINS-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        "# Foxins OpenSea kit\n\n"
        f"{count:,} flattened bold-graphic fox loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "3. Upload `FOXINS-opensea-drop.csv` (or `opensea-metadata.csv`). "
        ...
        "`public/foxins-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    if args.hats_only:
        ...
        print(f"Baking {count} Foxins tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Foxins GIFs to generated/foxins/gifs ({gib:.2f} GiB)")
