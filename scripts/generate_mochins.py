#!/usr/bin/env python3
"""Compose Mochins tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 4,000 on the shared 16-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/mochins-traits/.
"""
...
from build_mochins import (  # noqa: E402
    DURATION_MS,
    FRAMES,
    PREVIEW_DIR,
    SIGNATURES,
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "MOCHINS-opensea-drop.csv"):
...
def main() -> None:
    ...
    print(f"Wrote {count} Mochins GIFs to generated/mochins/gifs ({gib:.2f} GiB)")
