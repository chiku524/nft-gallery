#!/usr/bin/env python3
"""Paint Foxins — Style 5 bold graphic fox, layered like Shook'ums.
...
TRAIT_DIR = ROOT / "public" / "foxins-traits"
PREVIEW_DIR = ROOT / "public" / "foxins-preview"
...
COLLECTION_DESCRIPTION = (
    "Foxins is a 5,555-piece collection of looping bold-graphic fox PFP GIFs. "
    "Each Foxin is stacked from six layers — field, pelt, mug, hat, wrap, and charm — then flattened onto one 12-frame GIF. "
        ...
        samples.append(
            {
                "id": index,
                "name": f"Foxin #{index}",
                "image": f"/foxins-preview/{index}.gif",
                "attributes": [
...
def write_ts_gallery(samples: list[dict]) -> None:
    ...
    (SRC_DATA / "foxin-gallery.ts").write_text(
        "export type FoxinSample = {\n"
        "  id: number;\n"
        "  name: string;\n"
        "  image: string;\n"
        "  attributes: { trait_type: string; value: string }[];\n"
        "};\n\n"
        "export const foxinSamples: FoxinSample[] = [\n"
        + ",\n".join(rows)
        + "\n];\n",
        encoding="utf-8",
    )
...
def write_collection_meta() -> None:
    ...
    (META_DIR / "foxins-description.txt").write_text(COLLECTION_STORY + "\n", encoding="utf-8")
    (META_DIR / "foxins.json").write_text(
        json.dumps(
            {
                "name": "Foxins",
                "symbol": "FOXI",
                "description": COLLECTION_DESCRIPTION,
                "image": "/brand/collection-foxins.gif",
                "featured_image": "/brand/featured-foxins.jpg",
                "banner_image": "/brand/banner-foxins.png",
                "opensea_banner_image": "/brand/banner-foxins-opensea.jpg",
                "external_link": "/foxins",
...
def build_brand() -> None:
    ...
    save_image(logo.resize((512, 512), Image.Resampling.LANCZOS), BRAND_DIR / "logo-foxins.png")
    save_apng(
        [frame.resize((512, 512), Image.Resampling.LANCZOS) for frame in logo_frames],
        BRAND_DIR / "logo-foxins-loop.png",
    )
    ...
    save_image(fox_lineup(1500, 560, portraits[:5]).convert("RGB"), BRAND_DIR / "banner-foxins.png", quality=94)
    save_image(fox_lineup(2800, 700, portraits).convert("RGB"), BRAND_DIR / "banner-foxins-opensea.jpg", quality=90)
    ...
    save_image(featured.convert("RGB"), BRAND_DIR / "featured-foxins.jpg", quality=90)

    gif_frames = [frame.resize((1000, 1000), Image.Resampling.LANCZOS) for frame in logo_frames]
    save_loop_gif(gif_frames, BRAND_DIR / "collection-foxins.gif", DURATION_MS, colors=64)
...
def main() -> None:
    ...
    if args.brand_only:
        print("Writing Foxins brand kit…")
        build_brand()
        print("Done.")
        return
    if args.hats_only:
        print("Rebuilding Foxins hat layers…")
        ...
        print("Done.")
        return
    print("Building Foxins Style 5 graphic trait loops…")
    build_traits()
    print("Compositing sample GIF tokens…")
    build_samples()
    print("Writing brand…")
