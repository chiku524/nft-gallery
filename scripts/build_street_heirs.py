...
SYMBOL = "HEIRS"
SLUG = "street-heirs"
SUBJECT = "Clean-cartoon streetwear portraits with restrained graphic signals"
...
MINT = MINT_PRICE
TOKEN = "Street Heir"
ART_VERSION = "street-heirs-v1"
SEED = "street-heirs/production/v1"
COLLECTION_STORY = (
    "Street Heirs celebrates personal style, neighborhood craft, and the graphic language "
    "of the city. Authentic hair, tailoring, jewelry, and expression meet restrained "
    "broadcast signals in 5,555 clean-cartoon portraits on Robinhood Chain."
)
...
TRAIT_LABELS = tuple((category, category.title()) for category in TRAIT_ORDER)
...
def paint_adornment(trait: str) -> Image.Image:
    ...
    silver = "#c6d5dc"
    if trait == "gold-hoops":
        for coords, stud_x in (((130, 276, 168, 328), 146), ((344, 276, 382, 328), 360)):
            draw.arc(box(coords), 20, 340, fill=INK, width=width(8))
            draw.arc(box(coords), 20, 340, fill=gold, width=width(5))
            ellipse(draw, (stud_x, 276, stud_x + 10, 286), gold, INK, 2)
    elif trait == "silver-studs":
        for x in (140, 354):
            ellipse(draw, (x, 284, x + 18, 302), silver, INK, 3)
            ellipse(draw, (x + 5, 288, x + 10, 293), PAPER)
    elif trait == "curb-chain":
        links = ((174, 400), (202, 410), (230, 418), (258, 418), (286, 410), (314, 400))
        for x, y in links:
            coords = box((x, y, x + 38, y + 25))
            draw.ellipse(coords, outline=INK, width=width(9))
            draw.ellipse(coords, outline=gold, width=width(5))
        ellipse(draw, (250, 434, 264, 448), gold, INK, 2)
    elif trait == "pearl-line":
        pearls = ((184, 404), (211, 414), (240, 420), (270, 420), (299, 414), (326, 404))
        line(draw, [(192, 412), (220, 423), (250, 429), (280, 429), (308, 423), (335, 412)], INK, 4)
        for x, y in pearls:
            ellipse(draw, (x, y, x + 20, y + 20), PAPER, INK, 3)
            ellipse(draw, (x + 5, y + 4, x + 10, y + 9), "#ffffff")
    elif trait == "nose-ring":
        draw.arc(box((262, 292, 290, 326)), 300, 110, fill=INK, width=width(7))
        draw.arc(box((262, 292, 290, 326)), 300, 110, fill=gold, width=width(4))
    elif trait == "ear-cuff":
        for y in (272, 294):
            draw.arc(box((348, y, 380, y + 28)), 80, 282, fill=INK, width=width(7))
            draw.arc(box((348, y, 380, y + 28)), 80, 282, fill=silver, width=width(4))
...
def name_of(category: str, trait: str) -> str:
    return next(name for trait_id, name, _weight in TRAIT_SPEC[category] if trait_id == trait)
...
def compose(
    combo: dict[str, str],
    cache: dict[tuple[str, str], Image.Image] | None = None,
) -> Image.Image:
    canvas = Image.new("RGBA", (SIZE, SIZE), (0, 0, 0, 0))
    if combo["signal"] in BACK_SIGNAL_TRAITS:
        order = ("atmosphere", "signal", *STACK[1:-1])
    else:
        order = STACK
    for category in order:
        if cache is not None:
            canvas.alpha_composite(cache[(category, combo[category])])
        else:
...
def write_manifest() -> None:
    manifest = {
        ...
        "stack": list(STACK),
        "signal_placement": {
            trait_id: "behind-character" if trait_id in BACK_SIGNAL_TRAITS else "foreground"
            for trait_id, _, _ in TRAIT_SPEC["signal"]
        },
        "traits": {
            category: [
...
def write_trait_catalog() -> None:
    category_ids = " | ".join(json.dumps(category) for category in TRAIT_ORDER)
    labels = dict(TRAIT_LABELS)
    incompatible = [
        {
            "left": {"category": left[0], "trait": left[1]},
            "right": {"category": right[0], "trait": right[1]},
        }
        for left, right in INCOMPATIBLE
    ]
    lines = [
        ...
        f"export type StreetHeirsCategoryId = {category_ids};",
        "export type StreetHeirsTraitCategory = { id: StreetHeirsCategoryId; label: string; optional: false; traits: StreetHeirsTrait[] };",
        f"export const STREET_HEIRS_ART_VERSION = {json.dumps(ART_VERSION)};",
        f"export const streetHeirsStack = {json.dumps(STACK)} as const;",
        f"export const streetHeirsBackSignals = {json.dumps(sorted(BACK_SIGNAL_TRAITS))} as const;",
        "export const streetHeirsTraitCategories: StreetHeirsTraitCategory[] = [",
    ]
    for category in TRAIT_ORDER:
        lines.append(
            f"  {{ id: {json.dumps(category)}, label: {json.dumps(labels[category])}, optional: false, traits: ["
        )
        for trait_id, name, weight_value in TRAIT_SPEC[category]:
            image = f"/{SLUG}-traits/{category}/{trait_id}.png"
            placement = ""
            if category == "signal":
                value = (
                    "behind-character"
                    if trait_id in BACK_SIGNAL_TRAITS
                    else "foreground"
                )
                placement = f", placement: {json.dumps(value)}"
            lines.append(
                f"    {{ id: {json.dumps(trait_id)}, name: {json.dumps(name)}, "
                f"image: {json.dumps(image)}, weight: {weight_value}{placement} }},"
            )
        lines.append("  ] },")
    default_selection = combo_from_signature(SIGNATURES[0])
    lines += [
        "];",
        "export type StreetHeirsSelection = Record<StreetHeirsCategoryId, string>;",
        f"export const defaultStreetHeirsSelection: StreetHeirsSelection = {json.dumps(default_selection)};",
        f"export const streetHeirsIncompatiblePairs = {json.dumps(incompatible)} as const;",
    ...
    gallery = [
        "// Generated by scripts/build_street_heirs.py. Do not edit by hand.",
        "",
        "export type StreetHeirsSample = { id: number; name: string; image: string; attributes: { trait_type: string; value: string }[] };",
        "export const streetHeirsSamples: StreetHeirsSample[] = [",
    ]
    for token_id, signature in enumerate(SIGNATURES, 1):
        ...
        gallery.append(
            f"  {{ id: {token_id}, name: {json.dumps(f'{TOKEN} #{token_id}')}, "
            f"image: {json.dumps(f'/{SLUG}-preview/{token_id}.png?v={ART_VERSION}')}, "
            f"attributes: {json.dumps(attributes)} }},"
        )
    gallery += ["];",""]
    (DATA_ROOT / f"{SLUG}-gallery.ts").write_text(
        "\n".join(gallery),
        encoding="utf-8",
    )
...
def brand_font(size: int) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
    for name in ("C:/Windows/Fonts/arialbd.ttf", "arialbd.ttf", "DejaVuSans-Bold.ttf"):
        try:
...
def write_brand_assets() -> None:
    ...
    with Image.open(PREVIEW_ROOT / "1.png") as first:
        collection = first.convert("RGB").copy()
    save_image(collection, BRAND_ROOT / f"collection-{SLUG}.png", optimize=True)

    logo = Image.new("RGB", (512, 512), "#202A3D")
    logo_draw = ImageDraw.Draw(logo)
    logo_draw.ellipse((52, 52, 460, 460), outline="#D3B36C", width=12)
    logo_draw.text((256, 218), "SH", anchor="mm", font=brand_font(178), fill="#F4EFE5")
    logo_draw.text((256, 352), "STREET HEIRS", anchor="mm", font=brand_font(34), fill="#D3B36C")
    save_image(logo, BRAND_ROOT / f"logo-{SLUG}.png", optimize=True)

    with Image.open(PREVIEW_ROOT / "5.png") as portrait:
        featured = Image.new("RGB", (900, 600), "#D4AE70")
        featured_draw = ImageDraw.Draw(featured)
        featured_draw.text((58, 86), "STREET", font=brand_font(72), fill="#202A3D")
        featured_draw.text((58, 160), "HEIRS", font=brand_font(72), fill="#202A3D")
        featured_draw.rectangle((60, 258, 330, 270), fill="#80564A")
        featured_draw.text((60, 300), "5,555 PORTRAITS", font=brand_font(28), fill="#202A3D")
        paste_square(featured, portrait, 380, 0, 600)
    save_image(featured, BRAND_ROOT / f"featured-{SLUG}.jpg", quality=94, optimize=True)

    banner = Image.new("RGB", (1600, 600), "#202A3D")
    banner_draw = ImageDraw.Draw(banner)
    banner_draw.text((80, 170), "STREET", font=brand_font(104), fill="#F4EFE5")
    banner_draw.text((80, 270), "HEIRS", font=brand_font(104), fill="#D3B36C")
    banner_draw.text((86, 410), "STYLE IS INHERITED. IDENTITY IS MADE.", font=brand_font(28), fill="#A9BBC7")
    for index, token_id in enumerate((2, 4, 8)):
        with Image.open(PREVIEW_ROOT / f"{token_id}.png") as portrait:
            paste_square(banner, portrait, 720 + index * 300, 0, 600)
    save_image(banner, BRAND_ROOT / f"banner-{SLUG}.png", optimize=True)

    opensea = Image.new("RGB", (1400, 400), "#496A9A")
    opensea_draw = ImageDraw.Draw(opensea)
    opensea_draw.text((68, 128), "STREET HEIRS", font=brand_font(76), fill="#F4EFE5")
    opensea_draw.text((72, 228), "CLEAN CARTOON PORTRAITS / ROBINHOOD CHAIN", font=brand_font(24), fill="#EAD49F")
    for index, token_id in enumerate((1, 3, 6)):
        with Image.open(PREVIEW_ROOT / f"{token_id}.png") as portrait:
            paste_square(opensea, portrait, 760 + index * 220, -20, 440)
    save_image(
        opensea,
        BRAND_ROOT / f"banner-{SLUG}-opensea.jpg",
        quality=94,
        optimize=True,
    )
