#!/usr/bin/env python3
"""Compose Perfin tokens from layered APNG traits.

Default: 16 signature samples.
Pass --all to shuffle the full 8,888 on the shared 12-frame clock.

OpenSea gets GIFs only (max 10 GB). Studio traits stay 512×512 APNGs in public/perfin-traits/.
"""
...
from build_perfin import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        "name": f"Perfin #{token_id}",
        "description": "A looping engraved postage stamp from Perfin. Intaglio hatch. Walking cancel. One clock. Free mint on Robinhood Chain.",
        "image": f"{token_id}.gif",
        "attributes": attributes,
        "animation_loop": True,
        "compiler": "Perfin layer stack",
    }
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str], total_bytes: int) -> None:
    for name in ("opensea-metadata.csv", "PERFIN-opensea-drop.csv"):
    ...
    (OUT / "README.md").write_text(
        "# Perfin OpenSea kit\n\n"
        f"{count:,} flattened engraved-stamp loops at {DROP_SIZE}×{DROP_SIZE}, "
        ...
        "3. Upload `PERFIN-opensea-drop.csv` (or `opensea-metadata.csv`). "
        ...
        "`public/perfin-traits/` and are not the upload pack.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    print(f"Baking {count} Perfin tokens at {DROP_SIZE}px with {args.workers} workers…")
    ...
    print(f"Wrote {count} Perfin GIFs to generated/perfin/gifs ({gib:.2f} GiB)")
