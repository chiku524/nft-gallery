...
from build_ossein import (  # noqa: E402
    COLLECTION_STORY,
    DURATION_MS,
    FRAMES,
    GIF_COLORS,
...
def token_meta(token_id: int, selection: dict[str, str]) -> dict:
    ...
    return {
        ...
        "description": f"A looping anatomical chart from {NAME}. Ivory bone. Hairline ink. Free mint on Robinhood Chain.",
...
def write_sidecar(count: int, rows: list[dict], stats: Counter[str]) -> None:
    ...
    (OUT / "README.md").write_text(
        f"# {NAME} OpenSea kit\n\n"
        f"{count:,} flattened anatomical-chart loops at {SIZE}×{SIZE}, {FRAMES} frames, {DURATION_MS}ms"
